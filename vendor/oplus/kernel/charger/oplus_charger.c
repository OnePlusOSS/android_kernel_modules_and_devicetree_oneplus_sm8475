		...
		char __user *buff, size_t count, loff_t *off)
{
	...
	chg_err("cp_ibus = %d slave_cp_ibus = %d cp_vbat_deviation = %d\n", cp_ibus, slave_cp_ibus, cp_vbat_deviation);
...
static void oplus_chg_get_battery_data(struct oplus_chg_chip *chip)
{
	...
	if (chip->charger_exist && oplus_chg_show_vooc_logo_ornot())
		...
		if(chip->quick_mode_stop_time == 0) {
...
static void oplus_chg_fast_switch_check(struct oplus_chg_chip *chip)
{
	...
	chg_err("[%d, %d, %d, %d, %d] [%d, %d, %d, %d, %d]\n",
		chip->vooc_project, chip->mmi_chg, chip->pd_authentication, reset_mcu_delay, chip->pd_wait_svid,
		mcu_update, oplus_pps_get_chg_status(), chip->pps_force_svooc, chip->pps_svooc_enable, chip->chg_ops->oplus_chg_get_pd_type());
...
/*10 seconds after typec init, PD starts to communicate, so set true force reset adapter.only for MTK*/
	...
	if (chip->charger_type == POWER_SUPPLY_TYPE_USB_DCP) {
		if (oplus_vooc_get_fastchg_started() == false
			...
			if (oplus_pps_get_support_type() == PPS_SUPPORT_2CP && oplus_chg_get_voocphy_support() == ADSP_VOOCPHY && chip->pps_svooc_enable == false) {
				/*oplus_adsp_voocphy_reset();*/
				chip->pps_svooc_enable = true;
			}
			oplus_vooc_switch_fast_chg();
			mcu_status = 1;
		}
...
static void oplus_chg_pps_config(struct oplus_chg_chip *chip)
{
	...
	if (oplus_pps_get_ffc_started() == true) {
		chg_err(", oplus_pps_get_ffc_started == true\n");
		oplus_chg_unsuspend_charger();
		return;
	}
...
int oplus_chg_show_vooc_logo_ornot(void)
{
	...
	if (oplus_vooc_get_fastchg_started()) {
		...
		return 1;
			...
			|| oplus_vooc_get_fastchg_dummy_started() == true
			|| oplus_vooc_get_adapter_update_status() == ADAPTER_FW_NEED_UPDATE) {
		if ((g_charger_chip->vooc_project == 1 || g_charger_chip->vooc_project == 5 || g_charger_chip->vooc_project == 9)
			&& g_charger_chip->prop_status == POWER_SUPPLY_STATUS_FULL
...
void oplus_first_enable_adsp_voocphy(void)
{
	...
	chg_err("first_enabled_adspvoocphy:%d", g_charger_chip->first_enabled_adspvoocphy);
	...
	if (g_charger_chip->first_enabled_adspvoocphy != false) {
		...
		oplus_adsp_voocphy_turn_off();
		oplus_chg_suspend_charger();
		chg_err("1111 first_enabled_adspvoocphy:%d", g_charger_chip->first_enabled_adspvoocphy);
		msleep(1500);
		oplus_chg_unsuspend_charger();
		chg_err("2222 first_enabled_adspvoocphy:%d", g_charger_chip->first_enabled_adspvoocphy);
		msleep(500);
		chg_err("3333 first_enabled_adspvoocphy:%d", g_charger_chip->first_enabled_adspvoocphy);
		oplus_adsp_voocphy_turn_on();
		return;
	}
}
