...
static struct oplus_chg_track_type enhance_type_table[] = {
	...
	{ CHARGER_SUBTYPE_PPS, TRACK_POWER_MW(150000), "pps" },
	...
	struct oplus_chg_track_status *track_status)
{
	...
	if(track_status->hyper_en == 1) {
		...
		index += snprintf(&(p_trigger_data->crux_info[index]),
				OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
				"$$hyper_stop_cap@@%d",
				track_status->hyper_stop_soc);

		index += snprintf(&(p_trigger_data->crux_info[index]),
				OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
				"$$hyper_stop_temp@@%d",
				track_status->hyper_stop_temp);
		...
		index += snprintf(&(p_trigger_data->crux_info[index]),
				OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
				"$$hyper_est_save_t@@%d",
				track_status->hyper_est_save_time);
...
static int oplus_chg_track_init(struct oplus_chg_track *track_dev)
{
	...
	chip->track_status.hyper_en = false;
...
static int oplus_chg_track_cal_hyper_speed_status(
	struct oplus_chg_chip *chip,
	struct oplus_chg_track_status *track_status)
{
	if (!track_status || !chip)
	...
	track_status->hyper_last_time =
		chip->quick_mode_stop_temp - chip->quick_mode_start_time;
	track_status->hyper_ave_speed =
		(chip->quick_mode_stop_cap - chip->quick_mode_start_cap) /
		(track_status->hyper_last_time / 60);
	track_status->hyper_est_save_time = chip->quick_mode_gain_time_ms/1000;
	if((chip->quick_mode_start_time != 0) && (track_status->hyper_en == false))
		track_status->hyper_en = true;
	if((chip->quick_mode_start_time == 0) && (track_status->hyper_en == true))
		track_status->hyper_en = false;
