		...
		union power_supply_propval *pval)
{
	...
	if (prop == POWER_SUPPLY_PROP_USB_TYPE) {
		chg_err("POWER_SUPPLY_PROP_USB_TYPE[%d]\n", pval->intval);
	}
...
static int oplus_get_subboard_temp(void)
{
	...
	int subboard_temp = 0;
	struct battery_chg_dev *bcdev = NULL;
	static int subboard_temp_volt_pre = USBTEMP_DEFAULT_VOLT_VALUE_MV;
	struct oplus_chg_chip *chip = g_oplus_chip;
	...
	if (IS_ERR_OR_NULL(bcdev->iio.subboard_temp_v_chan)) {
		...
		subboard_temp_volt = subboard_temp_volt_pre;
		goto exit;
	}
	...
	resistance_convert_temperature_855(subboard_temp_volt,
		subboard_temp, i, con_temp_volt_855);
...
/*#endif*/
	...
	chg_err("!!!usb_pre_plugin_status[%d], usb_plugin_status[%d], usb_in_status[%d]\n", usb_pre_plugin_status, usb_plugin_status, bcdev->usb_in_status);
...
int oplus_chg_get_charger_subtype(void)
{
	...
	if (charg_subtype == CHARGER_SUBTYPE_DEFAULT) {
		...
		pr_debug(" USB_ADAP_SUBTYPE=%d\n", pst->prop[USB_ADAP_SUBTYPE]);
	}
