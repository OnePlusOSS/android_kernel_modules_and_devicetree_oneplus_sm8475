...
void iris_dbg_info_get(void)
{
	...
	pr_err("iris test IRIS_RX1 VDO_META = %x, FC1 0x%x; FC0 0x%x\n", rc[0], rc[1], rc[2]);
}
...
void iris_pwil_0_regdump_get_debug(void)
{
	...
	iris_rx0_meta_get();
	iris_dbg_info_get();
	pr_err("iris test PWIL_0_CTRL0 = 0x%x", iris_pwil_0_ctrl0_get());
	pr_err("iris test PWIL_0_INPUT_META = 0x%x", iris_pwil_0_input_meta_ctrl_get());
	pr_err("iris test PWIL_0_disp_ctrl0 = 0x%x", iris_pwil_0_disp_ctrl0_get());
...
void iris_emv_exit_prepare(void)
{
	if (iris_emv_instant_send) {
		...
		iris_emv_pwil_shortpacket_mask(0);
		//capt_en need to wait until the display mode changed
		iris_emv_pwil_capt_enable(1);
	}
}
...
u32 iris_blending_intstat_get(void)
{
	...
	IRIS_LOGI("iris test BLENDING_INTSTAT = %x", rc);
	rc &= 0x8;
	return (rc == 0x8);
}
