...
void iris_control_pwr_regulator(bool on)
{
	...
	if (rc)
		...
		IRIS_LOGD("%s() power %s iris", __func__, on ? "on" : "off");
}
...
int iris_parse_subnode(struct dsi_display *display, void *node)
{
	int32_t rc = -EINVAL;
	struct iris_cfg *pcfg = iris_get_cfg();
...
int iris_parse_param(struct dsi_display *display)
{
	...
	if (lightup_node) {
		...
		ret = iris_parse_subnode(display, lightup_node);
	} else
...
static int iris_fw_parse_dts(struct dsi_display *display)
{
	char fw_name[256] = {};
	struct iris_dts_ops *p_dts_ops = iris_get_dts_ops();

	if (p_dts_ops && p_dts_ops->id == DTS_CTX_FROM_FW) {
...
void iris_prepare(struct dsi_display *display)
{
	...
	if (is_boot) {
		...
		if (pcfg->valid < PARAM_PARSED && iris_fw_parse_dts(display)) {
			pcfg->abyp_ctrl.abypass_mode = ANALOG_BYPASS_MODE;
			return;
		}
		if (pcfg->valid < PARAM_PREPARED) {
