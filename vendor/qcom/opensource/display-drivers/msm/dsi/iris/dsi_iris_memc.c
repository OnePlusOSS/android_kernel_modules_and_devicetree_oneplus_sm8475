...
void iris_mvf_reg_set(void)
{
	if (iris_low_latency_mode_get()) {
		...
		iris_frc_reg_add(IRIS_MVF_ADDR + MVF_HMTNFB, 0x003df000, 0);
...
void iris_memc_mspwil_setting_init(void)
{
	...
	pcfg->chip_mspwil_setting.memc_mode = MEMC_DISABLE;
...
void iris_pwil_reg_set(void)
{
	...
	iris_set_ipopt_payload_data(IRIS_IP_PWIL, 0x70, 2, payload[0]);
	iris_set_ipopt_payload_data(IRIS_IP_PWIL, 0x70, 3, payload[1]);
	...
	iris_set_ipopt_payload_data(IRIS_IP_PWIL, 0xB0, 7, payload[5]);
	iris_set_ipopt_payload_data(IRIS_IP_PWIL, 0xB0, 8, payload[6]);
	iris_set_ipopt_payload_data(IRIS_IP_PWIL, 0xB0, 9, payload[7]);
	...
	if (!frc_setting->sr_en)
		payload[3] |= (1 << 9);
	iris_set_ipopt_payload_data(IRIS_IP_PWIL, 0xf0, 4, payload[2]);
	iris_set_ipopt_payload_data(IRIS_IP_PWIL, 0xf0, 5, payload[3]);
...
static void iris_sr_pwil_update(bool enable, int processWidth, int processHeight, uint8_t chain)
{
	...
	iris_set_ipopt_payload_data(IRIS_IP_PWIL, 0xf0, 5, payload[0]);
	iris_init_update_ipopt_t(IRIS_IP_PWIL, 0xf0, 0xf0, 1);
	...
	iris_set_ipopt_payload_data(IRIS_IP_PWIL, 0xA0, 3, payload[1]);
	iris_set_ipopt_payload_data(IRIS_IP_PWIL, 0xA0, 7, payload[5]);
