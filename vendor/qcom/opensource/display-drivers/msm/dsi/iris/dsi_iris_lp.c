...
static void iris_abyp_register_dump(void)
{
	u32 value = 0;
	u32 index = 0;
	u32 len = 0;
	...
	for (index = 0; index < len; index++) {
		ret = iris_ioctl_i2c_read(iris_register_list[index], &value);
		if (ret) {
...
enter_abyp_begin:
	...
	if (abyp_status_gpio != 1) {
		...
		if (toler_cnt > 0) {
			...
			_iris_abyp_stop();
			pcfg->abyp_ctrl.abyp_failed = true;
			IRIS_LOGE("Enter abyp failed!", __LINE__, toler_cnt);
			rc = -1;
			return rc;
		}
	}
...
static int _iris_esd_check(void)
{
	...
	if (pcfg->ocp_read_by_i2c && pcfg->iris_i2c_read) {
		if (pcfg->iris_i2c_read(0xf1a00204, &value) < 0) {
			IRIS_LOGE("%s(): iris i2c read dcs cmd fail", __func__);
			rc = -1;
			goto exit;
		}
...
ssize_t _iris_esd_sysfs_read(char *buf, ssize_t count)
{
	int len = 0;
	struct iris_cfg *pcfg;
...
{
	return _iris_esd_sysfs_read(buf, SZ_32);
}
